...
io.on('connection', (socket) => {
 ...
 // socket.broadcast.emit emits an event to every single connection except for the one that triggered the event.
  ...
  socket.on('join', ({ username, room }, callback) => {
    ...
    // This is used to emit an event to every single connection that is currently in the user's room. So when a user joins a room, we want to emit an event to every single connection that is currently in that room to update the list of users in that room.
